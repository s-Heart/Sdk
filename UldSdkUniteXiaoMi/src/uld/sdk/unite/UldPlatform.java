...
//import uld.sdk.others.Sdk91;
//import uld.sdk.others.SdkQianbao;
//import uld.sdk.others.SdkAnZhi;
//import uld.sdk.others.SdkBaiDuDuoKu;
//import uld.sdk.others.SdkDianJin;
//import uld.sdk.others.SdkDianJinHelper;
//import uld.sdk.others.SdkIFeng;
//import uld.sdk.others.SdkJiFeng;
//import uld.sdk.others.SdkBaoruan;
//import uld.sdk.others.SdkJiYou;
//import uld.sdk.others.SdkShouYou;
//import uld.sdk.others.SdkSiKai;
//import uld.sdk.others.SdkThreeG;
//import uld.sdk.others.SdkUld;
//import uld.sdk.others.SdkUc;
//import uld.sdk.others.SdkWanDouJia;
import uld.sdk.others.SdkXiaoMi;
...
public class UldPlatform extends UldBase {
	...
	public void beforeInitGame(Activity context,
			uld.sdk.unite.GameInfo gameInfo,
			final OnInitFinishListener onInitFinishListener) {
			...
			public void onLoginFinished(LoginResult loginResult) {
				if (!isLogin) {
					...
					if (loginResult.isLogin) {
						loginResult.setSign(Utility.encodeMD5(gameInfo
								.getUldPid()
								+ "&"
	...
	private void basicInfoInit() {
		...
		if (Utility.isEmpty(sDeviceName) || sDeviceName.equals("000000000000000")) {
			...
			if (wifiManager != null) {
				...
				if (wifiInfo != null
						&& !uld.sdk.tools.Utility.isEmpty(wifiInfo
								.getMacAddress())) {
	...
	private void stat() {
		...
		// Remark字段：分辨率+ip地址：
		WindowManager windowManager = (WindowManager) sContext
				.getSystemService(sContext.WINDOW_SERVICE);
		String remark = windowManager.getDefaultDisplay().getWidth() + "_"
				+ windowManager.getDefaultDisplay().getHeight();
		remark += "|*|" + getLocalIPAddress();
		mobileDevice.setRemark(remark);

		MessageBody messageBody = new MessageBody(
				"uld.sdk.bll.StatisticAnalysis", "Stat", new Class<?>[] {
						int.class, int.class, int.class,
						wh.promotion.model.MobileDevice.class }, new Object[] {
						gameInfo.gameId, sChannelId, sChannelSubId,
						mobileDevice });
		MessageReturn messageReturn = ThreadManager.sendMessage(messageHeader,
				messageBody);
		uld.sdk.model.StatisticAnalysis sAnalysis = null;
		if (messageReturn.getRetObject() != null) {
			sAnalysis = (uld.sdk.model.StatisticAnalysis) messageReturn
					.getRetObject();
		}
		if (sAnalysis != null) {
	...
	// 充值
	public void recharge(GameInfo gameInfo, Activity context,
			final RechargeCallBackListener rechargeCallBackListener) {
	...
	public void gotoPlatform(Activity context) {
		...
		sContext = context;
		if (sChannelSubId == ChannelSubIdXiaoMi) {
			// to do nothing
		}
	}

	public void gotoPlatform(Context context,
			GotoPlatCallBackListener gotoPlatCallBackListener) {
	}
	...
	private void handoutLogin(LoginCallBackListener loginCallBackListener) {
		...
		if (sChannelSubId == ChannelSubIdXiaoMi) {
			SdkXiaoMi.getInstance().loginSDK(sContext, gameInfo,
					loginCallBackListener);
		}
	}

	// 渠道分发充值
	private void handoutRecharge(
			RechargeCallBackListener rechargeCallBackListener) {
		if (sChannelSubId == ChannelSubIdXiaoMi) {
			SdkXiaoMi.getInstance()
					.recharge(sContext, rechargeCallBackListener);
		}
	}

	// TODO: xiaomi Add exitGame, viewUser, logoutUser
}
