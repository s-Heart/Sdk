...
public class MainActivity extends Activity {
	...
	private void setListeners() {
		if (btnWelcome != null) {
			...
			if(btnLogout !=null){
				btnLogout.setOnClickListener(new GotofinishListener());
			}
			btngoto.setOnClickListener(new LogoutListener());
	}
	...
	// 退出程序
	class GotofinishListener implements OnClickListener {
		...
		public void onClick(View v) {
			uld.sdk.unite.UldPlatform.getInstance().logout(MainActivity.this,new LogoutCallBackListener() {
				...
				public void onLogoutFinished(LogoutResult logoutResult) {
					if (logoutResult.getIsLogout()==true){
						//游戏方自行处理
						Toast.makeText(getApplicationContext(), "注销成功", Toast.LENGTH_LONG).show();
						Log.d(getPackageName(), "注销成功");
					} else {
						//注销失败
						Toast.makeText(getApplicationContext(), "注销失败！错误信息："+logoutResult.getLogoutErrorMsg(), Toast.LENGTH_LONG).show();
						Log.d(getPackageName(), "注销失败");
					}
				}
			});	
		}
