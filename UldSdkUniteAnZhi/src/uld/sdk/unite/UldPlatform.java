...
public class UldPlatform extends UldBase {
	...
	public void gotoPlatform(Activity context) {
		sContext = context;
		if (sChannelSubId == ChannelSubIdAnzhi) {
			SdkAnZhi.getInstance().viewUser(context);
		}
	}
	...
	private void handoutInit() {
		if (sChannelSubId == ChannelSubIdAnzhi) {
			SdkAnZhi.getInstance().initSDK(sContext);
		}
	}
	...
	private void handoutLogin(LoginCallBackListener loginCallBackListener) {
		...
		if (sChannelSubId == ChannelSubIdAnzhi) {
			SdkAnZhi.getInstance().login(sContext, gameInfo, loginCallBackListener);
		}
	...
	private void handoutRecharge(RechargeCallBackListener rechargeCallBackListener) {
		if (sChannelSubId == ChannelSubIdAnzhi) {
			SdkAnZhi.getInstance().recharge(sContext, rechargeCallBackListener);
		}
	}

	public void gotofinish(Context context) {
		if (sChannelSubId == ChannelSubIdAnzhi) {
			SdkAnZhi.getInstance().finishGame(context);
		}
	}
	...
	public void gotoCancellation(Context context) {
		if (sChannelSubId == ChannelSubIdAnzhi) {
			SdkAnZhi.getInstance().Cancellation(context);
		}
	}

	public void gotoSetSwitcheCallBack(SwitcheCallBackListener switcheCallBackListener) {
		if (sChannelSubId == ChannelSubIdAnzhi) {
			SdkAnZhi.getInstance().setSwitcheCallBack(switcheCallBackListener);
		}
	}
