...
import uld.sdk.others.Sdk17173;
...
public class UldPlatform extends UldBase {
	...
	// 17173Sdk
	private final static int ChannelSubId17173 = 104;
	...
	public void gotoFinish(Activity sContext) {
		if (sChannelSubId == ChannelSubId17173) {
			Sdk17173.getInstance().gotofinish(sContext);
		}
	}
	...
	private void handoutInit() {
		if (sChannelSubId == ChannelSubId17173) {
			Sdk17173.getInstance().initSDK(sContext);
		}
	}
	...
	private void handoutLogin(LoginCallBackListener loginCallBackListener) {
		if (sChannelSubId == ChannelSubId17173) {
			Sdk17173.getInstance().loginSDK(sContext, gameInfo, loginCallBackListener);
		}
	}
	...
	private void handoutRecharge(RechargeCallBackListener rechargeCallBackListener) {
		if (sChannelSubId == ChannelSubId17173) {
			Sdk17173.getInstance().recharge(sContext, rechargeCallBackListener);
		}
	}
	...
	private void handoutLogout(Context sContext, LogoutCallBackListener logoutCallBackListener) {
		if (sChannelSubId == ChannelSubId17173) {
			Sdk17173.getInstance().logoutSDK(sContext, logoutCallBackListener);
		}
	}
