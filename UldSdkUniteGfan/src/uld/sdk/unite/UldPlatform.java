...
public class UldPlatform extends UldBase {
	...
	private final static int ChannelSubIdGfan = 36;
	 ...
	 */
	public void login(Activity context, GameInfo gameInfo, final LoginCallBackListener loginCallBackListener) {
		sInstance.sContext = context;
		sInstance.gameInfo = gameInfo;
		isLogin = false;
	...
	public void gotoFinish(Activity sContext) {
		if (sChannelSubId == ChannelSubIdGfan) {
			SdkGfan.getInstance().gotofinish(sContext);
		}
	}
	...
	private void handoutInit() {
		if (sChannelSubId == ChannelSubIdGfan) {
			SdkGfan.getInstance().initSDK(sContext);
		}
	}
	...
	private void handoutLogin(LoginCallBackListener loginCallBackListener) {
		if (sChannelSubId == ChannelSubIdGfan) {
			SdkGfan.getInstance().loginSDK(sContext, gameInfo, loginCallBackListener);
		}
	}
	...
	private void handoutRecharge(RechargeCallBackListener rechargeCallBackListener) {
		if (sChannelSubId == ChannelSubIdGfan) {
			SdkGfan.getInstance().recharge(sContext, rechargeCallBackListener);
		}
	}
	...
	private void handoutLogout(Context sContext, LogoutCallBackListener logoutCallBackListener) {
		if (sChannelSubId == ChannelSubIdGfan) {
			SdkGfan.getInstance().logoutSDK(sContext, logoutCallBackListener);
		}
	}
