...
public class UldPlatform extends UldBase {
	...
	public void gotoPlatform(Activity context) {
		sContext = context;
		if (sChannelSubId == ChannelSubId91) {
			NdCommplatform.getInstance().ndEnterPlatform(0, sContext);
		}
	}
	...
	private void handoutLogin(LoginCallBackListener loginCallBackListener) {
		...
		if (sChannelSubId == ChannelSubId91) {
			Sdk91.getInstance()
					.login(sContext, gameInfo, loginCallBackListener);
		}
	}
	...
	private void handoutRecharge(
		...
		if (sChannelSubId == ChannelSubId91) {
			Sdk91.getInstance().recharege(rechargeCallBackListener);
		}
	}
