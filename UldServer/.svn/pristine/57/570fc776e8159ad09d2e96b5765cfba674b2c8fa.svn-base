...
public class SignInLog
{
	 ...
	 * @return 签道记录编号
	 */
	public int createUpdate(wh.member.model.SignInLog model) throws Exception{
		Map<String, Object> inParameters = new HashMap<String, Object>();
		inParameters.put("SignInLogId", model.getSignInLogId());
		...
		int retValue = SQLHelper.runProcedureNonQuery(Config.CONNECTION_STRING_ULD,
			"{? = call WH_Member_SignInLog_CreateUpdate(" + sbBuilder.toString() + ")}", inParameters, outParameters);		
	 ...
	 * @return 签道记录
     */
	public wh.member.model.SignInLog get(int id) throws Exception{
		wh.member.model.SignInLog model = null;
		Map<String, Object> inParameters = new HashMap<String, Object>();
		inParameters.put("SignInLogId", id);
		...
		String errMsg = "";
		try {
			rs = SQLHelper.runProcedure(Config.CONNECTION_STRING_ULD, "{call WH_Member_SignInLog_Get(?,?,?)}", inParameters, outParameters);
			if (rs != null) {
				rs.next();
				model = getModel(rs);
	 ...
	 * @return 签道记录列表
     */
	public List<wh.member.model.SignInLog> getList(wh.member.model.SignInLog queryModel, RefObject<Integer> totalCount, 
		boolean isAll, int pageIndex, int pageSize, String fldSort, int sortType) throws Exception {
		List<wh.member.model.SignInLog> list = new ArrayList<wh.member.model.SignInLog>();
		...
		inParameters.put("SignInLogId", queryModel.getSignInLogId());
		...
		String errMsg = "";
		try {
			rs = SQLHelper.runProcedure(Config.CONNECTION_STRING_ULD, "{call WH_Member_SignInLog_GetList(" + sbBuilder.toString() + ")}",
	...
	public void remove(int id) throws Exception{
		...
		try {
			SQLHelper.runProcedureNonQuery(Config.CONNECTION_STRING_ULD, "{call WH_Member_SignInLog_Remove(?,?,?)}", inParameters, outParameters);
		} catch (Exception e) {
			throw new Exception(e.getMessage(), e);
		}
	 ...
	 * @return
	 */
	protected static wh.member.model.SignInLog getModel(ResultSet rs) throws SQLException{
		wh.member.model.SignInLog m = null;
		if (rs != null)
        {
            m = new wh.member.model.SignInLog();
			try{
				m.setSignInLogId(rs.getInt("SignInLogId"));
