...
/**
 * 文档版本：v3.3 SDK版本：v3.3 最后更新：2014-4-2 15:18:44</br>
 * 
 * @author 史少杰
 * 
 */
public class UserGfan {
	/** token验证接口地址 */
	private static String TOKEN_URL = "http://api.gfan.com/uc1/common/verify_token";
	...
	public MessageReturn login(final int gameId, final int ServerId, int mobileDeviceId, int statisticAnalysisId, final int channelId,
		...
		try {
			...
			returnJson= HttpRequest.sendGet(url, param);
		} catch (Exception e) {
		...
		if (returnJson != null) {
			if (returnJson.contains("uid")) {
				final String uid = returnJson.substring(returnJson.indexOf("uid")+5,returnJson.length()-1);
				// 返回成功
				...
				new Thread() {
					public void run() {
						LogHelper.log("UserGfan->login thread begin" + String.valueOf(ServerId));
						UserUnite.getInstance().login(gameId, ServerId, channelId, channelSubId, uid, fmobileDeviceId,
								fstatisticAnalysisId);
						LogHelper.log("UserGfan->login thread end" + String.valueOf(ServerId));
					}
				}.start();
