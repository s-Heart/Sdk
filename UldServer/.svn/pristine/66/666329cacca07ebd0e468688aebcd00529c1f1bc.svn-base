...
public class Province
{
	 ...
	 * @return 省编号
	 */
	public int createUpdate(wh.shop.model.Province model) throws Exception{
		...
		int retValue = SQLHelper.runProcedureNonQuery(Config.CONNECTION_STRING_ULD,
			"{? = call WH_Shop_Province_CreateUpdate(" + sbBuilder.toString() + ")}", inParameters, outParameters);		
	 ...
	 * @return 省
     */
	public wh.shop.model.Province get(int id) throws Exception{
		wh.shop.model.Province model = null;
		Map<String, Object> inParameters = new HashMap<String, Object>();
		inParameters.put("ProvinceId", id);
		...
		String errMsg = "";
		try {
			rs = SQLHelper.runProcedure(Config.CONNECTION_STRING_ULD, "{call WH_Shop_Province_Get(?,?,?)}", inParameters, outParameters);
			if (rs != null) {
				rs.next();
				model = getModel(rs);
	 ...
	 * @return 省列表
     */
	public List<wh.shop.model.Province> getList(wh.shop.model.Province queryModel, RefObject<Integer> totalCount, 
		boolean isAll, int pageIndex, int pageSize, String fldSort, int sortType) throws Exception {
		List<wh.shop.model.Province> list = new ArrayList<wh.shop.model.Province>();
		...
		String errMsg = "";
		try {
			rs = SQLHelper.runProcedure(Config.CONNECTION_STRING_ULD, "{call WH_Shop_Province_GetList(" + sbBuilder.toString() + ")}",
	...
	public void remove(int id) throws Exception{
		...
		try {
			SQLHelper.runProcedureNonQuery(Config.CONNECTION_STRING_ULD, "{call WH_Shop_Province_Remove(?,?,?)}", inParameters, outParameters);
		} catch (Exception e) {
			throw new Exception(e.getMessage(), e);
		}
	 ...
	 * @return
	 */
	protected static wh.shop.model.Province getModel(ResultSet rs) throws SQLException{
		wh.shop.model.Province m = null;
		if (rs != null)
        {
            m = new wh.shop.model.Province();
			try{
				m.setProvinceId(rs.getInt("ProvinceId"));
