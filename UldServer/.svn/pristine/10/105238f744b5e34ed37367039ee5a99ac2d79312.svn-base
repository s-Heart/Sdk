...
public class User17173 {
	...
	public MessageReturn login(final int gameId, final int ServerId, int mobileDeviceId, int statisticAnalysisId, final int channelId,
			final int channelSubId, String token, String sMobileDeviceName) {
		...
		if (returnJSON != null) {
			if (returnJSON.contains("errorcode")) {
				...
				new Thread() {
					public void run() {
						LogHelper.log("User17173->checkSid thread begin" + String.valueOf(ServerId));
						UserUnite.getInstance().login(gameId, ServerId, channelId, channelSubId, usergameid, fmobileDeviceId,
								fstatisticAnalysisId);
						LogHelper.log("User17173->checkSid thread end" + String.valueOf(ServerId));
					}
				}.start();
