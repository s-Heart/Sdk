...
public class UserBaiDuDuoKu {

	public MessageReturn login(int gameId,int serverId,int mobileDeviceId,int statisticAnalysisId,
			int channelId, int channelSubId,String sid, final String uid,String sMobileDeviceName) {
		Date date = new Date();
	...
	public MessageReturn createOrder(wh.order.model.Order model, String channelUserId, int channelId, int channelSubId, String playerId) {
		...
		model.setServerId(ServerTools.getServerIdBySequenceNumber(model.getGameId(), model.getServerId(), "UserBaiDuDuoKu-createOrder"));
