...
public class ServerAccount
{
	 ...
	 * @return 服务器帐户编号
	 */
	public int createUpdate(wh.server.model.ServerAccount model) throws Exception{
		...
		int retValue = SQLHelper.runProcedureNonQuery(Config.CONNECTION_STRING_ULD,
			"{? = call WH_Server_ServerAccount_CreateUpdate(" + sbBuilder.toString() + ")}", inParameters, outParameters);		
	 ...
	 * @return 服务器帐户
     */
	public wh.server.model.ServerAccount get(int id) throws Exception{
		wh.server.model.ServerAccount model = null;
		Map<String, Object> inParameters = new HashMap<String, Object>();
		inParameters.put("ServerAccountId", id);
		...
		String errMsg = "";
		try {
			rs = SQLHelper.runProcedure(Config.CONNECTION_STRING_ULD, "{call WH_Server_ServerAccount_Get(?,?,?)}", inParameters, outParameters);
			if (rs != null) {
				rs.next();
				model = getModel(rs);
	...
	public List<wh.server.model.ServerAccount> getList(wh.server.model.ServerAccount queryModel, RefObject<Integer> totalCount, 
		...
		String errMsg = "";
		try {
			rs = SQLHelper.runProcedure(Config.CONNECTION_STRING_ULD, "{call WH_Server_ServerAccount_GetList(" + sbBuilder.toString() + ")}",
	...
	public void remove(int id) throws Exception{
		...
		try {
			SQLHelper.runProcedureNonQuery(Config.CONNECTION_STRING_ULD, "{call WH_Server_ServerAccount_Remove(?,?,?)}", inParameters, outParameters);
		} catch (Exception e) {
			throw new Exception(e.getMessage(), e);
		}
	 ...
	 *        状态：0、状态；1、有效；2、无效；
	 */
	public void modifyStatus(int serverAccountId, byte status) throws Exception{
		Map<String, Object> inParameters = new HashMap<String, Object>();
		inParameters.put("ServerAccountId", serverAccountId);
		...
		try {
			SQLHelper.runProcedureNonQuery(Config.CONNECTION_STRING_ULD, "{call WH_Server_ServerAccount_ModifyStatus(?,?,?)}", inParameters, outParameters);
		} catch (Exception e) {
			throw new Exception(e.getMessage(), e);
		}
	 ...
	 * @return
	 */
	protected static wh.server.model.ServerAccount getModel(ResultSet rs) throws SQLException{
		wh.server.model.ServerAccount m = null;
		if (rs != null)
        {
            m = new wh.server.model.ServerAccount();
			try{
				m.setServerAccountId(rs.getInt("ServerAccountId"));
				...
				wh.server.model.SACategory sACategory = new wh.server.model.SACategory();
				sACategory.setSACategoryId(rs.getInt("SACategory_SACategoryId"));
				sACategory.setManager(new wh.member.model.Manager());
				sACategory.getManager().setManagerId(rs.getInt("SACategory_ManagerId"));
				sACategory.setSACategoryName(rs.getString("SACategory_SACategoryName"));
				sACategory.setLevel(rs.getInt("SACategory_Level"));
				sACategory.setCreateDate(rs.getTimestamp("SACategory_CreateDate"));
				sACategory.setStatus(rs.getByte("SACategory_Status"));
				m.setSACategory(sACategory);
