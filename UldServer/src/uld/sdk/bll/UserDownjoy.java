...
public class UserDownjoy {
	...
	public MessageReturn login(int gameId, int ServerId, int mobileDeviceId, int statisticAnalysisId, int channelId, int channelSubId,
			String memberId, String token, String sMobileDeviceName) {
		Date date = new Date();
	...
	public MessageReturn login(final int gameId, final int ServerId, final int mobileDeviceId, final int statisticAnalysisId,
		...
		if (returnJSON != null) {
			if (returnJSON.contains("error_msg")) {
				...
				new Thread() {
					public void run() {
						LogHelper.log("UserDownjoy->login thread begin" + String.valueOf(ServerId));
						UserUnite.getInstance().login(gameId, ServerId, channelid, channelSubId, memberId, mobileDeviceId,
								statisticAnalysisId);
						LogHelper.log("UserDownjoy->login thread end" + String.valueOf(ServerId));
					}
				}.start();
