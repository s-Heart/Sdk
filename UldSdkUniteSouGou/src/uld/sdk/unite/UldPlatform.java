...
import uld.sdk.others.SdkSouGou;
...
public class UldPlatform extends UldBase {
	...
	// 17173Sdk
	private final static int ChannelSubIdSouGou = 121;
...
//				sChannelSubName = channelDatas[2];
	...
	public void gotoFinish(Activity sContext) {
		if (sChannelSubId == ChannelSubIdSouGou) {
			SdkSouGou.getInstance().gotofinish(sContext);
		}
	}
	...
	public void gotoPlatform(Activity sContext) {
		if (sChannelSubId == ChannelSubIdSouGou) {
			SdkSouGou.getInstance().gotoPlatform(sContext);
		}
	}
	...
	private void handoutInit() {
		if (sChannelSubId == ChannelSubIdSouGou) {
			SdkSouGou.getInstance().initSDK(sContext); 
		}
	}
	...
	private void handoutLogin(LoginCallBackListener loginCallBackListener) {
		if (sChannelSubId == ChannelSubIdSouGou) {
			SdkSouGou.getInstance().loginSDK(sContext, gameInfo, loginCallBackListener);
		}
	}
	...
	private void handoutRecharge(RechargeCallBackListener rechargeCallBackListener) {
		if (sChannelSubId == ChannelSubIdSouGou) {
			SdkSouGou.getInstance().recharge(sContext, rechargeCallBackListener);
		}
	}
	...
	private void handoutLogout(Context sContext, LogoutCallBackListener logoutCallBackListener) {
		if (sChannelSubId == ChannelSubIdSouGou) {
			SdkSouGou.getInstance().logoutSDK(sContext, logoutCallBackListener);
		}
	}
