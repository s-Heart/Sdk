...
import uld.sdk.others.SdkLenovo;
...
public class UldPlatform extends UldBase {
	...
	// 17173Sdk
	private final static int ChannelSubIdLenovo = 82;
...
//				sChannelSubName = channelDatas[2];
	...
	public void gotoFinish(Activity sContext) {
		if (sChannelSubId == ChannelSubIdLenovo) {
			SdkLenovo.getInstance().gotofinish(sContext);
		}
	}
	...
	public void gotoPlatform(Activity sContext) {
		if (sChannelSubId == ChannelSubIdLenovo) {
			SdkLenovo.getInstance().gotoPlatform(sContext);
		}
	}
	...
	private void handoutInit() {
		if (sChannelSubId == ChannelSubIdLenovo) {
			SdkLenovo.getInstance().initSDK(sContext); 
		}
	}
	...
	private void handoutLogin(LoginCallBackListener loginCallBackListener) {
		if (sChannelSubId == ChannelSubIdLenovo) {
			SdkLenovo.getInstance().loginSDK(sContext, gameInfo, loginCallBackListener);
		}
	}
	...
	private void handoutRecharge(RechargeCallBackListener rechargeCallBackListener) {
		if (sChannelSubId == ChannelSubIdLenovo) {
			SdkLenovo.getInstance().recharge(sContext, rechargeCallBackListener);
		}
	}
	...
	private void handoutLogout(Context sContext, LogoutCallBackListener logoutCallBackListener) {
		if (sChannelSubId == ChannelSubIdLenovo) {
			SdkLenovo.getInstance().logoutSDK(sContext, logoutCallBackListener);
		}
	}
