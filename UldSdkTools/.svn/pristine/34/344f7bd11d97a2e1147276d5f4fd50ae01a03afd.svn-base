...
public class ChannelSubEnum {
	...
	public enum ChannelSubStatus	{
		...
		ChannelSubStatus(byte value) {
			this.value = value;
		}

		public static ChannelSubStatus parse(byte value) {
			ChannelSubStatus retValue = ChannelSubStatus.状态;
			for (ChannelSubStatus item : ChannelSubStatus.values()) {
				if (value == item.getValue()) {
					retValue = item;
					break;
				}
